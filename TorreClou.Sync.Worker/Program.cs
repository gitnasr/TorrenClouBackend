...
try
{
    Log.Information("Starting sync worker application");
...
builder.Services.AddHangfireServer(options =>
{
    ...
    options.Queues = ["sync", "default"];
});

// Register Job Services (Hangfire jobs)
builder.Services.AddScoped<S3SyncJob>();
...
// 2. Sync Worker - Redis stream consumer for sync jobs
...
{
    Log.Fatal(ex, "Sync worker application terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
